...
class semanticRoadMap:
    def __init__(self, lsi_serializationAddress, dictionary_serialAddress, pns_serialAddress, index_serialAddress,corpus_serialAddress):
        ...
        :param index_serialAddress: absolute path of index serialization file
        :param corpus_serialAddress: 'corpus.data',it contains all nodes' texts,even more than that,but never less
    ...
    def getSimByNodeID(self,nodeID,sims):
        '''
        this function is used in getMaxWeightPathBySingleNode_graph_nodeWeight(), getMaxWeightPathBySingleNode_nodeWeight, getMaxWeightPathBySingleNode_graph_nodeWeight_arcWeight, getMaxWeightPathBySingleNode_nodeWeight_raw
        return similarity between text of nodeID and the interested string
        :param nodeID: integer,index of self.g.nodes()
        :param sims: a list containing similarities between corpus and the query text
        :return: float
        '''
    ...
    def getSimByNode_pnIndex(self, node_pnIndex, sims):
        ...
        :param node_pnIndex: node index in pn list
        :param sims: a list containing similarities between corpus and the query text
        :return: float
        '''
    ...
    def getNodeIDByNodePN(self, NodePN):
        '''
        return nodeID by node's patent NO
        :param NodePN: string
        :return: int
        '''
        result = None
    ...
    def getMaxWeightPathInGraph_returnGraphType_nodeWeight(self, sims):
        '''
        this function uses node weight, which usually is the similarity between
        query text and docs in corpus, to generate query based global main path
        :param sims: list, containing the similarities between query text and docs in corpus
        :return: a list of sorted max weight paths induced by all source nodes
        '''
        l = []
    ...
    def getMaxWeightPathInGraph_nodeWeight_arcWeight(self, sims,node_weight=1.0,arc_weight=1.0):
        '''
        this function use node weight and arc weight to generate query based global main path
        Args:
            :param node_weight: relative weight of node
            :param arc_weight: relative weight of arc
            :param sims: a list containing the similarites between the query text and every doc in corpus.
        Returns:
    ...
    def calculateSimilarityByTerms(self, terms):
        '''
        calculate similarities between terms and self.corpus
        :param terms:a list of splitted text
    ...
    def computeArcWeight4GraphByTextSim(self, g, pns):
        '''
        set textual similarity as arc weight in g
        :param g: DiGraph
        :param pns: a list of patent NO
        :return: None
        '''
    ...
    def setArcWeight4GrahphByLSI(self,texts):
        ...
        :param texts: a list of splitted string
        :return: None
        '''
        arcs = self.g.edges()
        for arc in arcs:
    ...
    def addLSIWeight2TopologyWeight4ArcInGraph(self,texts,topology_weight=1.0,text_weight=1.0):
        ...
        :param texts: a list of splitted text
        :param topology_weight: weight of topological arc weight in main path searching, such as SPC(Search Path Count), SPNC(Search Path Node Pair)
        :param text_weight: weight of textual similarity in main path searching
        :return: None
        '''
        arcs = self.g.edges()
        for arc in arcs:
    ...
    def setArcWeight4GrahphByLSIViaSimMatrix(self,corpus):
        ...
        :param corpus: load corpus.data into memory
        :return: None
        '''
        arcs = self.g.edges()
        for arc in arcs:
    ...
    def create_sim_matrix(self,model,corpus):
        ...
        :param model: trained gensim.model, i.e., lsi_serialization
        :param corpus: 'corpus.data'
        :return: np 2D array
        '''
    ...
    def queryBasedMainPath_nodeWeight(self,query_text,topN_graph=1):
        """Create query based main path according to node weight
        Args:
